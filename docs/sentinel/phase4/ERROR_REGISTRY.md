# Error Registry — Basamak-1 (24 Kod)
...
## 3. Kod → LLD bağlaması (24/24 — kod 24 phase5.5'te eklendi)
...
| 10 | `RES_FAILURE_EVENT_CORRELATION_MISS` | RES | WARN+metrik+eşik-alarmı | **`FailureEventCorrelationMissConsumer`** (Flowable `EventRegistryNonMatchingEventConsumer` SPI — *phase5.5 düzeltmesi 2026-07-15: `eventReceived` no-match'te exception ATMAZ [TEST_SPEC d kanıtı], eski `catch` dalı ölü koddu; gerçek tetik artık SPI*) | WARN | trace_id, correlation_key, channel | `nats.flowable.failure_event.correlation_miss` — bkz. §4.2 |
...
| 24 | `VAL_INVALID_REPLY_TYPE` | VAL | WARN+DLQ-route | `A2ReplyPayloadDecoder.classify()` — *yeni kod (CODER-Q1 kararı 2026-07-15): reply'da zorunlu `type: SUCCESS\|BPMN_ERROR\|TRANSIENT` discriminator'ı eksik/bilinmeyen → DLQ (`DlqReason.INVALID_REPLY_TYPE`)* | WARN | trace_id, external_task_id, subject | `nats.jetstream.inbound.dlq` (dlqCount) |
