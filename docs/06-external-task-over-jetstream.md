...
- **D-E — DLQ/ack semantiği:** iki idiom için ortak mı, idiom-özel mi? (§7) — D-D yarısını cevapladı: **substrat mekaniği ortak** (maxDeliver→`dlq.<subject>`), **DLQ-sonrası işleme idiom-özel** (Camunda incident / Flowable failure-event). Kalan: ack disiplini + DLQ subject/payload şemasının ortak wire-contract'a bağlanması.
