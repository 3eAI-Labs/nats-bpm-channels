...
*(D-E rafinesi 2026-07-14: worker `nakWithDelay` kullanıyorsa teslimat bütçesi backoff toplamı kadar uzar → şemsiye koşulu genel haliyle **L ≥ M·W + Σbackoff + S + ε**. Defaults'ta Σbackoff = 1+2+4 = 7s → alt sınır **307s**; default **L=320s**. İlk yazımdaki L=300s bu koşulu 7s ihlal ediyordu — phase-review MAJOR-B düzeltmesi, 2026-07-14. **BAQ-3:** topic-özel L bu alt sınırı ihlal ederse deployment-time **hard-reject** — topic aktivasyonu engellenir; bilinçli kaçış yalnız `allow-unsafe-lock-duration=true` flag'i + kalıcı WARN ile.)*
...
3. **Bütçe bitti:** deliveryCount > M → DLQ subject'i + bridge `handleFailure(..., retries=0, retryDuration=0)` → **incident (Cockpit görünürlüğü)**. *(BAQ-2: retryDuration=0 → lockExpiration=now; Cockpit-retry artık-kilit beklemeden akar — `SetExternalTaskRetriesCmd` lockExpiration'a dokunmaz.)* `retries=0` task fetchable-predicate dışı → sweep onu ASLA yeniden yayınlamaz (`ExternalTask.xml:222`); operatör Cockpit'ten retry verirse task yeniden fetchable olur → sweep doğal olarak yeniden yayınlar (Cockpit-retry JetStream'e geri akar).
...
**Metodoloji — Testcontainers yük-bench modülü (basamak-1 teslimatına DAHİL):** PG + engine + NATS + N simüle worker; **aynı senaryo iki modda** koşar (native-poll baseline ↔ A2-push), `@Tag("bench")`, CI'da nightly/manuel. **Kabul kriteri (PO-Q7 netleştirmesi, 2026-07-14):** sert kapı YALNIZ birincil metriktir — poll + fetchAndLock bileşenleri **0**, INSERT/complete bileşenleri **artmıyor**. Destekleyici SLI'lar (dispatch p95 ≤ 200ms dahil) **izlenen hedeflerdir**, kapanışı bloklamaz (bench donanım/CI gürültüsüne duyarlı → sert kapı yapılırsa flaky-fail üretir); regresyonları raporlanır.
...
- **Subject şeması:** `jobs.<type>` / event channel subject'leri; `*.reply` / inbound channel. *(BAQ-4, 2026-07-14):* `jobs.*` ve `dlq.jobs.*` önekleri **A2'ye rezervedir** — A2-dışı bir channel bu öneklerle subject tanımlarsa deployment-time validasyon (`VAL_TOPIC_NAMESPACE_COLLISION`) **engeller** (yanlış-bridge DLQ yönlendirmesi yapısal olarak imkansız).
...
- **DLQ / ack semantiği (D-E çözüldü 2026-07-14 — ortak kontrat, idiom-özel post-DLQ):**
  ...
  - **Topoloji:** TEK ortak `DLQ` stream (`dlq.>`), retention limits-based (default 14 gün, **kiracı-bazlı konfigürable** — PII işleyen kiracı kısaltmalı/erişimi kısıtlamalı, PO-Q3 2026-07-14), WorkQueue DEĞİL; tüketiciler subject filtresiyle ayrışır: `dlq.jobs.>` → Camunda incident-bridge, event-channel DLQ'ları → Flowable failure-event bridge; ops tek yüzeyden izler. Idiom-başına ayrı stream **reddedildi** (2× provisioning/izleme; tek wire-contract sadeliğinden sapma).
  - **Post-DLQ idiom-özel (D-D):** Camunda → `handleFailure(retries=0)` → incident; Flowable → failure-event.
  - **dlq-of-dlq YOK:** bridge DLQ mesajını işleyemezse nak + alert; asla ack-drop. *(BAQ-6, 2026-07-14):* DLQ-bridge'lerin engine çağrıları **nak-backoff + circuit-breaker** ile korunur — CB açıkken mesajlar stream'de beklemede kalır (kalıcı), uzun engine kesintisi DLQ-fırtınası üretmez.
  - ⚠️ **Mevcut adapter'larda kontrata aykırı üç açık (basamak-1 fix listesi):** (1) `publishToDlq` header kopyalamıyor — yalnız `msg.getData()` (flowable `JetStreamInboundEventChannelAdapter.java:218,227` / cadenzaflow `JetStreamMessageCorrelationSubscriber.java:210,219`) → correlation key'siz DLQ mesajından failure-event bridge correlate EDEMEZ; (2) DLQ-publish başarısızlığı yutulup koşulsuz `msg.ack()` yapılıyor (flowable `:141-145` / cadenzaflow `:123-127`), `dlqSubject==null` iken discard → **sessiz poison-mesaj kaybı** (custody-transfer ihlali); (3) DLQ publish'te `Nats-Msg-Id` yok → çift DLQ kaydı riski; (4) *(PO-Q2, 2026-07-14)* trace-header tutarsızlığı — inbound MDC `X-Trace-Id` okuyor (`JetStreamInboundEventChannelAdapter.java:119`) ama standart `X-Cadenzaflow-Trace-Id` (`BpmHeaders.java:12`) → geçiş kuralı: okuma tarafı iki adı da kabul eder (fallback), yazma tarafı yalnız standart adı yazar; (5) *(BAQ-5, 2026-07-14)* boş mesaj gövdesi sessizce DEBUG+ack'leniyor (`JetStreamInboundEventChannelAdapter.java:124-131`) → producer bug'ını gizler; yeni davranış: `VAL_EMPTY_MESSAGE_BODY` → **WARN + DLQ'ya yönlendir** (iz kaybolmaz, metrik artar).
- **Header'lar:** mevcut `BpmHeaders` (`X-Cadenzaflow-Trace-Id`, `-Business-Key`, `-Idempotency-Key`, async: `-Correlation-Id`, `-Reply-Subject`). *Güvenlik notu (normatif değil — PO-Q4, 2026-07-14):* `Business-Key` telco bağlamında MSISDN/abone-id olabilir → hash/mask önerilir; karar kiracının.
...
- **D-E — DLQ/ack semantiği:** ✅ **ÇÖZÜLDÜ (2026-07-14)** = **ortak kontrat + idiom-özel post-DLQ**: custody-transfer ack ilkesi (ack yalnız kalıcılık el değiştirince); in-band `maxDeliver+1` tespiti (advisory reddedildi); DLQ şeması = orijinal payload+header'lar aynen + `X-*-Dlq-*` meta + `Nats-Msg-Id=<orijinal>.dlq`; TEK `DLQ` stream (`dlq.>`, limits, 14g); dlq-of-dlq yok. Mevcut adapter'larda fix-listesi **5 kontrat açığı** (header kaybı, koşulsuz ack, dedup yok; PO-Q2 ile trace-header, BAQ-5 ile boş-body eklendi). D-B şemsiyesi rafine: `L ≥ M·W + Σbackoff + S + ε`. (§7)
...
> **Durum (2026-07-14):** D-A…D-F **tamamı çözüldü**; D-G/D-H bilinçli ertelendi. Bu belge **Sentinel phase1 girdisi olarak HAZIR.** Basamak-1 kod kapsamı: custom activity behavior + post-commit publisher (D-A/D-C), sweep (D-B), inbound completion-bridge, DLQ bridge'leri (D-D/D-E), §7 kontrat-fix listesi (5 açık), bench modülü (D-F).
