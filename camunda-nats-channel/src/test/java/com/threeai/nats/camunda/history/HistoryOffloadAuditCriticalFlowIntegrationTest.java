 ...
 * relays that row onto {@code history.camunda.INCIDENT.<processInstanceId>} and deletes it after
 * PubAck (BR-REL-001).
 */
@Testcontainers
class HistoryOffloadAuditCriticalFlowIntegrationTest {
    ...
    static void setUp() throws Exception {
        ...
        ClassCutoverStateRegistry cutoverRegistry = new ClassCutoverStateRegistry(kvManager, natsConnection, "camunda");
        ...
        handler = new NatsHistoryEventHandler(cutoverRegistry, classification, outboxWriter, postCommitPublisher,
                new DbHistoryEventHandler(), "camunda");
        ...
        SweepLeaderLease leaderLease = new SweepLeaderLease(jetStream, kvManager, natsConnection,
                leaderBucket, "relay-leader.", "camunda", "test-node", Duration.ofSeconds(60));
        assertThat(leaderLease.tryAcquireOrRenew()).isTrue();
        outboxRelay = new HistoryOutboxRelay(dataSource, jetStream, leaderLease, outboxProperties, null, "camunda");
    ...
    void createIncident_writesOutboxRowAndActHiRow_thenRelayPublishesAndDeletes() throws Exception {
        ...
        String subject = "history.camunda.INCIDENT." + processInstanceId;
