 ...
 *
 * <p><b>[BLOCKING] Sentinel Phase 5.5 QA finding (why this test is currently {@code @Disabled}):
 * </b> running this test against the REAL engine reproducibly throws — {@code
 * A2OrphanSweep.fetchFetchableParity()} passes {@code instructions.values()} (a
 ...
 * denial since JDK 16) — every invocation throws
 ...
 *
 * <p><b>Production impact:</b> {@code A2OrphanSweep.sweepCycle()}'s {@code fetchFetchableParity()}
 * call throws EVERY cycle on a standard JDK 21 launch; the {@code catch (Exception e)} in
 * {@code sweepCycle()} swallows it as {@code SYS_SWEEP_QUERY_FAILED} (plain ERROR, no
 * CRITICAL/page per {@code ERROR_REGISTRY.md} #13) and the cycle is skipped — meaning the ENTIRE
 * orphan-sweep safety net (ADR-0002/ADR-0003, the mechanism ADR-0001's umbrella-lock formula and
 * NFR-R3's "invisible orphan window &lt;= L" guarantee both depend on) silently never runs,
 * indefinitely, in a default deployment.
 *
 * <p><b>Suggested fix direction (NOT applied here — test-only QA scope):</b> materialize a plain
 * list before crossing into MyBatis/OGNL-reflected code, e.g.
 * {@code new java.util.ArrayList<>(instructions.values())}, in
 * {@code A2OrphanSweep.fetchFetchableParity()}. Re-enable this test once fixed — it is a real
 * regression guard for the DECISION_MATRIX Matrix-3 skip-branches, not a false positive.
 */
class A2OrphanSweepFetchableParityIntegrationTest {
    ...
    @Test
    @Disabled("BLOCKING (Sentinel Phase 5.5 QA finding, not a test defect): "
            + "A2OrphanSweep.fetchFetchableParity() passes a java.util.HashMap$Values view into "
            + "Camunda's MyBatis/OGNL-evaluated selectExternalTasksForTopics query, which throws "
            + "java.lang.reflect.InaccessibleObjectException on every JDK17+/21 launch without "
            + "--add-opens java.base/java.util=ALL-UNNAMED (reproduced independently of Camunda; "
            + "see class Javadoc). Orphan-sweep (ADR-0002/0003) never runs as a result. Re-enable "
            + "once nats-core/camunda-nats-channel coder wraps instructions.values() in a plain "
            + "ArrayList before passing it to the engine API.")
