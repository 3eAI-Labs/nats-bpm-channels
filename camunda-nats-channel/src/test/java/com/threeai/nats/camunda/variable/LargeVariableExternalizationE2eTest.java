...
/**
 * Real embedded Camunda engine (Testcontainers Postgres) + real content-addressed projection store
...
class LargeVariableExternalizationE2eTest {
    ...
    static void setUp() throws Exception {
        ...
        postCommitExternalizer = new LargeVariablePostCommitExternalizer(payloadStore, properties, null, "camunda");
        ...
        SweepLeaderLease leaderLease = new SweepLeaderLease(jetStream, kvManager, natsConnection,
                leaderBucket, "sweep-leader.", "camunda", "test-node", Duration.ofSeconds(60));
        sweep = new LargeVariableExternalizationSweep(
                engineDataSource, leaderLease, postCommitExternalizer, payloadStore, properties, "camunda");
        ...
        // Mirrors CamundaNatsAutoConfiguration's largeVariableProcessEnginePlugin bean, without Spring.
    ...
    void hardDeleteProcess_soleReference_reconciliationReleasesAndDeletesPayload() {
        ...
        assertThat(payloadStore.currentRuntimeReference("camunda", variableId)).isEmpty(); // ledger row cleaned up too
    }
    ...
    void hardDeleteProcess_sharedReference_survivesUntilLastReferrerDeleted() {
        ...
        assertThat(payloadStore.fetchByContentHash(contentHash)).isPresent();
        assertThat(payloadStore.currentRuntimeReference("camunda", variableIdA)).isEmpty();
        assertThat(payloadStore.currentRuntimeReference("camunda", variableIdB)).isPresent();
        ...
        assertThat(payloadStore.currentRuntimeReference("camunda", variableIdB)).isEmpty();
    }
