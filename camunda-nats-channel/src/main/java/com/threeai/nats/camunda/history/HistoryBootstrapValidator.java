...
/**
 * Bootstrap-time guard: {@code VAL_HISTORY_LEVEL_AUDIT_CRITICAL_MISMATCH} (BA-Q4 decision).
 * For every configured {@code auditCriticalClasses} member, checks whether the
 * engine's current {@code HistoryLevel} would actually PRODUCE that class's events — if not,
 * WARNs (does NOT fail-fast; increment 1's {@code UmbrellaLockValidator}/{@code VAL_UMBRELLA_LOCK_TOO_SHORT}
 * hard-reject pattern is a DELIBERATE non-match here, per BA-Q4).
 *
 * <p>Increment 1 {@code UmbrellaLockValidator}-style bootstrap-time-check pattern (not an
