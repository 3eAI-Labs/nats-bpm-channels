...
/**
 * QA review fix (HIGH, decision 2026-07-15) — {@code
 * EventReceivedNoMatchBehaviorTest} (a test source, not on the main javadoc classpath) proved
 * {@code EventRegistry.eventReceived(...)} does NOT
 ...
 * {@code RES_FAILURE_EVENT_CORRELATION_MISS} (BR-FLW-003/005) was
...
public class FailureEventCorrelationMissConsumer implements EventRegistryNonMatchingEventConsumer {
    ...
    public void handleNonMatchingEvent(EventRegistryEvent event, EventRegistryProcessingInfo processingInfo) {
        ...
        // BAQ-8: a single miss is a benign race (instance likely already
        // resolved via another path) — WARN + metric, not ERROR; the sustained-repeat alert
        // thresholds are what actually signal a systemic problem.
