...
/**
 * End-to-end audit-critical slice: real embedded CibSeven engine (Testcontainers Postgres — the
 ...
 * relays that row onto {@code history.cibseven.INCIDENT.<processInstanceId>} and deletes it after
 * PubAck (BR-REL-001).
 */
@Testcontainers
class HistoryOffloadAuditCriticalFlowIntegrationTest {
    ...
    static void setUp() throws Exception {
        ...
        ClassCutoverStateRegistry cutoverRegistry = new ClassCutoverStateRegistry(kvManager, natsConnection, "cibseven");
        ...
        handler = new NatsHistoryEventHandler(cutoverRegistry, classification, outboxWriter, postCommitPublisher,
                new DbHistoryEventHandler(), "cibseven");
        ...
        SweepLeaderLease leaderLease = new SweepLeaderLease(jetStream, kvManager, natsConnection,
                leaderBucket, "relay-leader.", "cibseven", "test-node", Duration.ofSeconds(60));
        assertThat(leaderLease.tryAcquireOrRenew()).isTrue();
        outboxRelay = new HistoryOutboxRelay(dataSource, jetStream, leaderLease, outboxProperties, null, "cibseven");
    ...
    void createIncident_writesOutboxRowAndActHiRow_thenRelayPublishesAndDeletes() throws Exception {
        ...
        String subject = "history.cibseven.INCIDENT." + processInstanceId;
