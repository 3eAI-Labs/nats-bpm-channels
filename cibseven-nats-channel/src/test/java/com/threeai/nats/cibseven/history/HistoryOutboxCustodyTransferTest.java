...
/**
 * BR-REL-001 — QA review: the three custody-transfer crash-points the audit-critical
 * outbox/relay design must survive without loss (NFR-R1). Real Postgres + real NATS JetStream
 * (Testcontainers) throughout; {@code HistoryOutboxRelayTest} already covers the
 * leader/non-leader/dedup DB-read paths this class does not re-test.
 */
@Testcontainers
class HistoryOutboxCustodyTransferTest {
    ...
    private HistoryOutboxRelay newRelayAsLeader() throws Exception {
        ...
        return new HistoryOutboxRelay(dataSource, jetStream, leaderLease, new HistoryOutboxProperties(), null, "cibseven");
    }

    /**
     * Crash-point 1 (before commit): a tx-injected failure AFTER {@code
    ...
    /**
     * Crash-point 2 (before publish / relay node restart): the outbox row is durable and
    ...
    /**
     * Crash-point 3 (before PubAck, broker outage): the real NATS container is Docker-paused
