...
class HistoryPostCommitPublisherTest {
    ...
    void publish_actinstEvent_landsOnHistorySubjectWithHeaders() throws Exception {
        ...
        publisher.publish(event, HistoryClassNames.ACTINST, "cibseven");

        JetStreamManagement jsm = natsConnection.jetStreamManagement();
        ConsumerConfiguration cc = ConsumerConfiguration.builder()
                .durable("test-consumer-" + UUID.randomUUID().toString().replace("-", ""))
                .filterSubject("history.cibseven.ACTINST.proc-bulk-1")
                .build();
        jsm.addOrUpdateConsumer("HISTORY_BULK_TEST", cc);
        JetStreamSubscription sub = jetStream.subscribe("history.cibseven.ACTINST.proc-bulk-1",
        ...
        assertThat(msg.getSubject()).isEqualTo("history.cibseven.ACTINST.proc-bulk-1");
    ...
    void publish_jetStreamUnavailable_doesNotThrow_atMostOnceLossAccepted() {
        ...
        try {
            ...
            org.assertj.core.api.Assertions.assertThatCode(
                    () -> publisher.publish(event, HistoryClassNames.ACTINST, "cibseven")).doesNotThrowAnyException();
        } catch (Exception e) {
    ...
    void asyncMode_ackIncrementsCounter_afterCompletion() throws Exception {
        ...
        org.cibseven.bpm.engine.impl.history.event.HistoricActivityInstanceEventEntity ev =
                new org.cibseven.bpm.engine.impl.history.event.HistoricActivityInstanceEventEntity();
        ev.setId("h-async-1");
        ev.setEventType("start");
        ev.setProcessInstanceId("pi-async");
        publisher.publish(ev, "ACTINST", "cibseven");
    ...
    void asyncMode_failure_doesNotThrow_sameToleranceAsSync() throws Exception {
        ...
        ev.setEventType("start");
        ev.setProcessInstanceId("pi-async");
        publisher.publish(ev, "ACTINST", "cibseven"); // atmaz
