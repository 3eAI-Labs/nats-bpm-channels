package com.threeai.nats.cibseven.outbound;
...
/**
 * End-to-end outbound-handoff slice (increment 4, decisions D-A'..D-F'): real embedded
 * CibSeven engine (Testcontainers Postgres) + real NATS JetStream (Testcontainers). The tenant BPMN
...
class NatsOutboundHandoffIntegrationTest {
    ...
    static void setUp() throws Exception {
        ...
        NatsOutboundPublisher publisher = new NatsOutboundPublisher(classification, outboxWriter, postCommitPublisher, "cibseven");
        ...
        config.setBeans(Map.of("natsOutboundPublisher", publisher, "noopDelegate",
                (org.cibseven.bpm.engine.delegate.JavaDelegate) execution -> { }));
    ...
    void criticalMessageThrow_writesOutboxRow_thenRelayPublishesAndDeletes() throws Exception {
        ...
        OutboundMessageRelay relay = new OutboundMessageRelay(dataSource, jetStream, leaderLease,
                new OutboundMessageOutboxProperties(), null, "cibseven");

        relay.relayCycle();

        assertThat(countOutboxRows(processInstanceId)).isZero();

        String subject = "events.cibseven.payment_requested." + processInstanceId;
    ...
    void bestEffortSendTask_publishesDirectly_zeroOutboxRows() throws Exception {
        ...
        String subject = "events.cibseven.order_created." + processInstanceId;
        List<Message> messages = fetchMessages(subject, "OUTBOUND_E2E_TEST");
        assertThat(messages).hasSize(1);
        assertThat(new String(messages.get(0).getData(), java.nio.charset.StandardCharsets.UTF_8))
