...
class HistoryWireMessageFactoryTest {
    ...
    void build_setsSubjectHeadersAndDedupId() {
        ...
        NatsMessage msg = HistoryWireMessageFactory.build("cibseven", "OP_LOG", "hist-evt-1", "create",
                "proc-1", "biz-1", fields, null, EVENT_TIME);

        assertThat(msg.getSubject()).isEqualTo("history.cibseven.OP_LOG.proc-1");
        assertThat(msg.getHeaders().getFirst(NatsJetStreamConstants.MSG_ID_HDR)).isEqualTo("hist-evt-1:create");
        assertThat(msg.getHeaders().getFirst(HistoryHeaders.ENGINE_ID)).isEqualTo("cibseven");
    ...
    void build_blankBusinessKey_headerOmitted() {
        NatsMessage msg = HistoryWireMessageFactory.build("cibseven", "ACTINST", "hist-evt-2", "start",
