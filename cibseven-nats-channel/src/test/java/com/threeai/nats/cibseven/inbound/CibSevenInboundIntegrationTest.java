...
@Testcontainers
class CibSevenInboundIntegrationTest {
    ...
    void inbound_coreNats_correlatesMessage() throws Exception {
        ...
        config.setSubject("cibseven.test.core");
        ...
        try {
            connection.publish("cibseven.test.core",
    ...
    void inbound_jetStream_acksAfterCorrelation() throws Exception {
        jsm.addStream(StreamConfiguration.builder()
                .name("CIBSEVEN-TEST")
                .subjects("cibseven.test.js")
                .build());

        SubscriptionConfig config = new SubscriptionConfig();
        config.setSubject("cibseven.test.js");
        config.setMessageName("JsTestMessage");
        config.setJetstream(true);
        config.setMaxDeliver(5);
        config.setDlqSubject("dlq.cibseven.test.js");
        ...
        try {
            jetStream.publish("cibseven.test.js",
    ...
    void inbound_jetStream_maxDeliverExceeded_dlq() throws Exception {
        jsm.addStream(StreamConfiguration.builder()
                .name("CIBSEVEN-DLQ")
                .subjects("cibseven.test.dlq")
                .build());

        jsm.addStream(StreamConfiguration.builder()
                .name("CIBSEVEN-DLQ-SINK")
                .subjects("dlq.cibseven.test.dlq")
                .build());
        ...
        SubscriptionConfig config = new SubscriptionConfig();
        config.setSubject("cibseven.test.dlq");
        config.setMessageName("DlqTestMessage");
        config.setJetstream(true);
        config.setMaxDeliver(2);
        config.setDlqSubject("dlq.cibseven.test.dlq");
        ...
        try {
            jetStream.publish("cibseven.test.dlq",
            ...
            PullSubscribeOptions pullOpts = PullSubscribeOptions.builder()
                    .stream("CIBSEVEN-DLQ-SINK")
                    .configuration(ConsumerConfiguration.builder()
                            .ackPolicy(AckPolicy.None)
                            .build())
                    .build();
            JetStreamSubscription dlqSub = jetStream.subscribe("dlq.cibseven.test.dlq", pullOpts);
