...
class CibSevenShardingAutoConfigurationTest {
    ...
    private ApplicationContextRunner runner(Connection connection) {
        return new ApplicationContextRunner()
                .withConfiguration(AutoConfigurations.of(CibSevenShardingAutoConfiguration.class))
    ...
    void gateOpen_validTopology_wiresTopologyRouterGuardPlugin() throws Exception {
        runner(validTopologyConnection())
                .withPropertyValues("spring.nats.cibseven.sharding.enabled=true",
                        "spring.nats.cibseven.sharding.shard-count=2",
                        "spring.nats.cibseven.sharding.shard-id=1",
                        "spring.nats.cibseven.sharding.routes[0].subject=evt.order.accept",
                        "spring.nats.cibseven.sharding.routes[0].business-key-field=orderId")
                .run(context -> {
    ...
    void gateOpen_validatorGatesBoot_missingStreamFailsContext() {
        ...
        runner(connection)
                .withPropertyValues("spring.nats.cibseven.sharding.enabled=true",
                        "spring.nats.cibseven.sharding.shard-count=2",
                        "spring.nats.cibseven.sharding.shard-id=0")
                .run(context -> {
    ...
    void gateOpen_invalidShardId_failsWithConfigCode() throws Exception {
        runner(validTopologyConnection())
                .withPropertyValues("spring.nats.cibseven.sharding.enabled=true",
                        "spring.nats.cibseven.sharding.shard-count=2",
                        "spring.nats.cibseven.sharding.shard-id=2")
                .run(context -> {
                    assertThat(context).hasFailed();
                    assertThat(context.getStartupFailure())
