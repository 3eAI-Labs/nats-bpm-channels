...
/**
 * Decision matrix 3 (sweep decision), rows 2/3/4 — {@code A2OrphanSweep}'s
 ...
 * {@code ACT_RU_EXT_TASK} rows in all four decision-matrix-3 states, then runs
 ...
 *
 * <p><b>[BLOCKING] QA review finding — FIXED (2026-07-15):</b>
 ...
 * {@code Map.values()} view, runtime type {@code java.util.HashMap$Values}) into CibSeven's
 ...
 * repo's build or its deployment documentation. Reproduced independently of
 * CibSeven/MyBatis (plain {@code HashMap.values()} + reflective {@code size()} call, same
 ...
 * {@code SYS_SWEEP_QUERY_FAILED} (plain ERROR, no CRITICAL/page) and the cycle was skipped —
 * meaning the ENTIRE orphan-sweep safety net (ADR-0002/ADR-0003, the mechanism ADR-0001's
 * umbrella-lock formula and NFR-R3's "invisible orphan window &lt;= L" guarantee both depend
 * on) silently never ran, indefinitely, in a default deployment.
 *
 ...
 * test is re-enabled and is a real regression guard for the decision-matrix-3
