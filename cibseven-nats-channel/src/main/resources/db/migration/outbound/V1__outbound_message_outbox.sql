...
-- Increment 4 -- locked decisions D-A'/D-C'/D-E'/D-F'
--
-- CODER-NOTE (implementation return report): the increment 4 design's module list places this table/migration under
-- "nats-core". This repo places it here instead (byte-mirrored into cadenzaflow-nats-channel),
-- following the SAME precedent compact_history_outbox already established in increment 2: an
...
-- schema migrations (deployment concern: Flyway/Liquibase changelog integration), same as
...
CREATE TABLE outbound_message_outbox (
    ...
    business_key        VARCHAR(255),                   -- CONFIDENTIAL/conditional PII; never in subject
...
-- Relay reads oldest-first (fairness + stuck-row age detection, increment 2 BA-Q7 precedent).
