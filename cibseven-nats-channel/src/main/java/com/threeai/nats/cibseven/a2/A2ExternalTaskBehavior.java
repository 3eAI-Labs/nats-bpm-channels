 ...
 * single flush later produces one INSERT, never a follow-up UPDATE (guard-tested). The
 * post-commit publish hook runs strictly after the transaction commits (COMMITTED listener,
 * {@code TransactionContext.java:49}) — outside the transaction, so JetStream publish latency
 * can never hold the engine DB transaction open.
 *
 * <p><b>In-tx variable capture (QA review fix, item 5, decision 2026-07-15):</b>
...
public class A2ExternalTaskBehavior extends ExternalTaskActivityBehavior {
    ...
    public void execute(ActivityExecution execution) throws Exception {
        ...
        //    Guard test (single-INSERT proof).
     ...
     * VariableScope} — no CibSeven static-context ({@code Context}/{@code CommandContext})
