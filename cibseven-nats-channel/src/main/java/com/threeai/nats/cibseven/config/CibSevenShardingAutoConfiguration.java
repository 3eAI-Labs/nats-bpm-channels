...
import com.threeai.nats.cibseven.shard.ShardBirthGuardParseListener;
 ...
 * {@code spring.nats.cibseven.sharding.enabled=true}: absent the gate NONE of these beans
 ...
 */
@AutoConfiguration(before = CibSevenNatsAutoConfiguration.class)
@ConditionalOnClass(org.cibseven.bpm.engine.ProcessEngine.class)
@ConditionalOnProperty(prefix = "spring.nats.cibseven.sharding", name = "enabled", havingValue = "true")
@EnableConfigurationProperties(CibSevenShardingProperties.class)
public class CibSevenShardingAutoConfiguration {

    @Bean
    public ShardTopology shardTopology(CibSevenShardingProperties properties) {
    ...
    public ShardBootstrapValidator shardBootstrapValidator(Connection connection,
            ShardTopology topology, CibSevenShardingProperties properties,
