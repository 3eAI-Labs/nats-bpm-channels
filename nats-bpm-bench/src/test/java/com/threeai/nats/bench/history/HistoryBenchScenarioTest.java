 ...
 * writes instead. {@code @Tag("bench")} — nightly/manual only.
 */
@Tag("bench")
class HistoryBenchScenarioTest {
    ...
    void historyOffloadProducesZeroActHiWritesWhileOutboxAbsorbsThem() {
        try (BenchEnvironment env = BenchEnvironment.start();
            ...
            // SYS_BENCH_HISTORY_ENVIRONMENT_UNAVAILABLE (FINDING-004, code review) -- the
            // increment 2-specific registry code (distinct from increment 1's generic
