...
public record Snapshot(List<QueryStat> queryStats) {
    ...
    /**
     * fetchable-parity predicate columns — the native poller's
     ...
     * ({@code fetchAndLock} is a SQL-signature FAMILY, since IN-list
    ...
    public DbRoundTripReport classify(BenchMode mode) {
        ...
        // Sweep read-count is amortized (<=1 read / S / cluster) — a single
