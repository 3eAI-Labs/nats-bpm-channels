 ...
 * baseline) and one A2 topic (A2ExternalTaskBehavior active).
 */
public class BenchEnvironment implements AutoCloseable {
    ...
    private BenchEnvironment() throws Exception {
        ...
        // Increment 2: compact_history_outbox must exist regardless of which BenchMode a given
    ...
    /** Increment 2: {@code HistoryBenchScenario} needs its OWN Postgres database on this SAME
    ...
    /** Increment 2: {@code HistoryBenchScenario} builds its own mode-specific engine(s) against
     ...
     * recommendation; see CODER-QUESTIONS in the implementation summary.
     */
    private static void ensureStreams(io.nats.client.Connection connection) throws Exception {
        ...
        jsm.addStream(StreamConfiguration.builder()
                ...
                // overlaps increment 2's separately-provisioned "dlq.history.>" (DLQ_HISTORY,
