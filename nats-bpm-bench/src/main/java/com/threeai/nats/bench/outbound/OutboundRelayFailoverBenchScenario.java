...
/**
 * {@code outbound-relay-leader} failover RTO/RPO measurement — increment 4 mirror of {@code
 * RelayFailoverBenchScenario} (increment 2's real-measurement precedent for {@code
 * history-relay-leader}), applied to {@code OutboundMessageRelay}/{@code outbound_message_outbox}
 * (increment 4, decisions D-A'/D-F'). Reuses the SAME {@link RelayFailoverReport} shape and
...
public class OutboundRelayFailoverBenchScenario {
    ...
    public RelayFailoverReport run(BenchEnvironment env, int engineNodeReplicaCount) {
        ...
        // BenchEnvironment only provisions the A2 job/reply/DLQ streams
        // -- OutboundMessageRelay publishes to "events.<engineId>.<type>.<processInstanceId>",
