 ...
 * — a config-like fixed map. Package-private — internal {@link ProjectionStore} plumbing, not an
 * LLD-listed public class.
 *
 * <p>Field keys are the exact camelCase keys the engine-side {@code HistoryEventFieldExtractor}
 * (`camunda-nats-channel`/`cadenzaflow-nats-channel`) emits (the wire {@code payload} is opaque to
 * this module; its JSON keys are the contract). Most
 ...
 * an error) — the wire payload may carry more fields than this increment's DB columns model.
 *
...
final class HistoryClassColumnMapping {
    ...
    private static final Map<String, TableMeta> TABLES = Map.ofEntries(
            // Entity-lifecycle (V1__entity_lifecycle_tables.sql)
            ...
            // Append-only log (V2__append_log_tables.sql) -- all 9 carry process_instance_id directly.
