 ...
 * Routes by class: audit-critical target →
 ...
 *
 * <p><b>CQ-3 (proposed — DP-10/FR-G2 primary bulk-PII erasure targets):</b> {@code
 ...
 * see {@code findVarinstLargePayloadIds} → {@code deleteLargePayloads}), TASKINST
 * {@code name}/{@code description}, and
...
public class ErasurePipeline {
     ...
     * ({@code deleted_at}/{@code anonymized_at} columns). Writes
    ...
    /**
     * increment 3 D-F': releases (rather than unconditionally hard-deletes) each large-payload
    ...
    /**
     * FINDING-002 (code review, MINOR): previously only re-checked {@code
     ...
     * TaskInstanceHistory} openapi schema does not carry {@code description} — this increment does
