...
/**
 * Wires the engine-neutral {@code nats-history-projection} module.
 ...
 *
 * <p><b>CQ-2/CQ-5 (proposed 2026-07-20 — multi-engine scope, resolved):</b> {@link
 * ReconciliationJob} and {@link RetentionEnforcementJob} each bind to exactly ONE {@code
 * engineId} ({@code history.engine-id}, default {@code "camunda"} — a property the config design
 * does not itself define). This stays AS-IS by design — the default auto-config
 ...
 * not oversight. Full recipe: {@code nats-history-projection/README.md} "Multi-Engine
 * Deployment" (CQ-2/CQ-5). The related engineId-not-in-query-API constraint is a SEPARATE, also
...
public class NatsHistoryProjectionAutoConfiguration {
     ...
     * secret path), not the literal key material. No OpenBao SDK dependency
     ...
     * tables), which this engine-neutral module does not itself own or create (the config design
