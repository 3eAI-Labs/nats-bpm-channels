...
-- Follows the audit-table convention for L3/L4/compliance-critical entities.
...
CREATE TABLE class_cutover_state (
    ...
    clean_streak_target   INTEGER      NOT NULL DEFAULT 7,       -- N (PO-Q4 default 7d, per-class config)
...
COMMENT ON TABLE class_cutover_state IS
    'Persistent per-class cutover state machine -- ReconciliationJob/CutoverControlPlane operational metadata, not PII (DP-14).';

-- ============================================================================
-- retention_audit_log -- compliance audit-table pattern. Compliance-critical: a FAILED
...
CREATE TABLE retention_audit_log (
    ...
    legal_basis       VARCHAR(255),                              -- e.g. 'KVKK v1.0 §4.2 (7y)' -- audit-critical classes
...
COMMENT ON TABLE retention_audit_log IS
    'Audit-log-per-deletion invariant: one row per retention deletion. Write failure after successful drop = SYS_RETENTION_AUDIT_LOG_WRITE_FAILED (CRITICAL page).';
...
-- ============================================================================
-- erasure_scope_confirmation -- BA-Q6 scope-confirmation flow (telco MSISDN churn protection).
-- ============================================================================
