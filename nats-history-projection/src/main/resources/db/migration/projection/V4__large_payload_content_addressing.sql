...
--                 LargeVariableExternalizer (camunda-nats-channel/cadenzaflow-nats-channel, increment 3)
-- Increment 3 (large-variable externalization):
...
-- logical content (RUNTIME ACT_RU_VARIABLE via the new increment 3 custom serializer, HISTORY
-- ACT_HI_DETAIL/ACT_HI_VARINST/EXT_TASK_LOG/ATTACHMENT via the existing increment 2 projection write
...
-- Backfill: every pre-existing row (increment 2, pre-dedup) gets its content hash computed from the
...
-- BEFORE the uniqueness constraint below is added -- increment 2 never deduplicated, so two rows with
...
-- increment 2 invariants, but the old ErasurePipeline.deleteLargePayloads/hard-delete path predates
...
COMMENT ON COLUMN projection_large_payload.content_hash IS
    'SHA-256 (hex) of payload_bytes -- increment 3 D-B''/D-D'' dedup key. Content-addressed: the SAME bytes from RUNTIME (ACT_RU_VARIABLE) and HISTORY (ACT_HI_DETAIL/ACT_HI_VARINST/EXT_TASK_LOG/ATTACHMENT) converge on one row.';
COMMENT ON COLUMN projection_large_payload.ref_count IS
    'increment 3 D-F'' refcount/GC -- number of live *_ref foreign keys pointing at this row. Row is deleted only when this reaches 0 (RetentionEnforcementJob/ErasurePipeline decrement it; never a direct per-caller DELETE).';
