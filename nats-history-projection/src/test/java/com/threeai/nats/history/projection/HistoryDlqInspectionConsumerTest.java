...
/**
 * QA review — {@link HistoryDlqInspectionConsumer} had ZERO test coverage before this class
 ...
 * inspection — subject-ACL is the primary defense, this
 * class is the CB-protected/DP-1-compliant fallback visibility path). A REAL {@link
 * CircuitBreaker} (increment 1 {@link DlqBridgeCircuitBreakerFactory}, same style as {@code
...
class HistoryDlqInspectionConsumerTest {
    ...
    void onMessage_circuitBreakerOpen_naksFastWithoutProcessing() {
        ...
        HistoryDlqInspectionConsumer consumer = new HistoryDlqInspectionConsumer(cb);
        // Trip the circuit breaker (5 consecutive failures -> OPEN, increment 1 ADR-0004 threshold).
        for (int i = 0; i < 5; i++) {
