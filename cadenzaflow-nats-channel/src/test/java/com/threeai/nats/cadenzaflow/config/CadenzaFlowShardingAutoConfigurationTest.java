...
class CadenzaFlowShardingAutoConfigurationTest {
    ...
    private ApplicationContextRunner runner(Connection connection) {
        return new ApplicationContextRunner()
                .withConfiguration(AutoConfigurations.of(CadenzaFlowShardingAutoConfiguration.class))
    ...
    void gateOpen_validTopology_wiresTopologyRouterGuardPlugin() throws Exception {
        runner(validTopologyConnection())
                .withPropertyValues("spring.nats.cadenzaflow.sharding.enabled=true",
                        "spring.nats.cadenzaflow.sharding.shard-count=2",
                        "spring.nats.cadenzaflow.sharding.shard-id=1",
                        "spring.nats.cadenzaflow.sharding.routes[0].subject=evt.order.accept",
                        "spring.nats.cadenzaflow.sharding.routes[0].business-key-field=orderId")
                .run(context -> {
    ...
    void gateOpen_validatorGatesBoot_missingStreamFailsContext() {
        ...
        runner(connection)
                .withPropertyValues("spring.nats.cadenzaflow.sharding.enabled=true",
                        "spring.nats.cadenzaflow.sharding.shard-count=2",
                        "spring.nats.cadenzaflow.sharding.shard-id=0")
                .run(context -> {
    ...
    void gateOpen_invalidShardId_failsWithConfigCode() throws Exception {
        runner(validTopologyConnection())
                .withPropertyValues("spring.nats.cadenzaflow.sharding.enabled=true",
                        "spring.nats.cadenzaflow.sharding.shard-count=2",
                        "spring.nats.cadenzaflow.sharding.shard-id=2")
                .run(context -> {
                    assertThat(context).hasFailed();
                    assertThat(context.getStartupFailure())
