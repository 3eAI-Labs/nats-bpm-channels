...
/**
 * Audit-critical tx-in writer (BR-HDL-003, ADR-0010 + ARCH-Q1). Called from
 * {@link NatsHistoryEventHandler#handleEvent} in the SAME transaction as the
 ...
 *
 * <p><b>CQ-1 (recommended — pseudonym-vault write ownership, ADR-0016 "persist
 ...
 * gap in the audit-critical flow itself — matching ADR-0016's explicit "an unreachable vault MUST
 * NOT BLOCK the AUDIT flow" contract). {@code vaultClient} is nullable — pseudonymization opt-in
...
public class CompactHistoryOutboxWriter {
    ...
    /** Increment 2 LLD-sketch-parity constructor — no vault wiring (pseudonymization opt-in without
    ...
    public void write(HistoryEvent historyEvent, String historyClass, String engineId,
        ...
        try (PreparedStatement insertOutbox = engineTxConnection.prepareStatement(INSERT_OUTBOX_SQL)) {
            ...
            // Engine-native tx-fail (NOT SYS_OUTBOX_RELAY_PUBLISH_FAILED --
