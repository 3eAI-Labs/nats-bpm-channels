...
/**
 * {@code spring.nats.cadenzaflow.history.outbox.*} — relay cycle period + stuck-row alarm multiplier
 * (LLD-Q5/BA-Q7). The increment 1 "ADR-0004 thresholds are fixed" pattern is DELIBERATELY
 * not repeated here — the stuck-threshold multiplier is left tenant/bench calibratable
 * (PO-Q4 pattern), per LLD-Q5.
 */
@ConfigurationProperties(prefix = "spring.nats.cadenzaflow.history.outbox")
public class HistoryOutboxProperties {

    /** relayCyclePeriod default 30s (validation deferred by design review, item #3, RTO derivation input). */
