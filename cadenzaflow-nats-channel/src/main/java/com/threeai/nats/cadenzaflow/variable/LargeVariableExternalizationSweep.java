...
/**
 * Leader-elected, catch-all externalization sweep (increment 3, decision D-A'
 * "post-commit/downstream" — the SLOW path, mirroring {@code HistoryOutboxRelay}'s scheduled-
 ...
 * failure, ...): the fast path is best-effort by design (D-A', increment 3 design), so this sweep
 ...
 * camunda-7.x-lineage (cadenzaflow fork, byte-identical schema — ADR-0007) physical column names
 * ({@code ID_}, {@code TYPE_}, {@code BYTEARRAY_ID_}, {@code
 ...
 *
 * <p><b>FINDING-001 fix (code review) — {@link #reconcileRuntimeReferences}:</b> D-F' refcount/GC
 ...
 * SAME increment 2 {@code ReconciliationJob} idiom (compare authoritative source vs. derived state,
...
public class LargeVariableExternalizationSweep {
    ...
    public void sweepCycle() {
        ...
        if (!leaderLease.tryAcquireOrRenew()) {
            return; // not the leader -- zero DB reads (increment 1/2 parity, ADR-0002)
        }
