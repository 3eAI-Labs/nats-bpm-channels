...
import com.threeai.nats.cadenzaflow.shard.ShardBirthGuardParseListener;
 ...
 * {@code spring.nats.cadenzaflow.sharding.enabled=true}: absent the gate NONE of these beans
 ...
 */
@AutoConfiguration(before = CadenzaFlowNatsAutoConfiguration.class)
@ConditionalOnClass(org.cadenzaflow.bpm.engine.ProcessEngine.class)
@ConditionalOnProperty(prefix = "spring.nats.cadenzaflow.sharding", name = "enabled", havingValue = "true")
@EnableConfigurationProperties(CadenzaFlowShardingProperties.class)
public class CadenzaFlowShardingAutoConfiguration {

    @Bean
    public ShardTopology shardTopology(CadenzaFlowShardingProperties properties) {
    ...
    public ShardBootstrapValidator shardBootstrapValidator(Connection connection,
            ShardTopology topology, CadenzaFlowShardingProperties properties,
