 ...
 * TypedValueSerializer} (increment 3, decisions D-A'/D-C'/D-E'). Registered
 ...
 * instance always wins over the built-in it wraps without needing any fork change (increment 3
 * design evidence).
 *
 ...
 * serializer would have written (D-A' "in-tx staging only = today's behavior"). Only if the
 ...
 * method (D-A': "synchronous-in-serializer FORBIDDEN").
 *
...
public class LargeVariableSerializer<T extends TypedValue> implements TypedValueSerializer<T> {
    ...
    public void writeValue(T value, ValueFields valueFields) {
        ...
        if (!(valueFields instanceof VariableInstanceEntity variableInstance)) {
            // D-C' (increment 3 design, item 4): RUNTIME process-variable scope only for this
            // increment (CMMN case-variables use a DIFFERENT ValueFields-implementing entity and
            // are a documented, bounded follow-up — see the implementation return report CODER-QUESTIONS).
            // Behaves exactly like the built-in serializer for any other ValueFields implementation.
            log.debug("Large value exceeds threshold on a non-RUNTIME-variable ValueFields — "
                    + "externalization not attempted (out of increment 3 scope)",
