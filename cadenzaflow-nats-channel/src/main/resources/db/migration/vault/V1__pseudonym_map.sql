...
-- Target database: PSEUDONYM VAULT Postgres (ARCH-Q2 decision 2026-07-18 -- separate instance/schema,
-- isolated from BOTH engine DB and projection DB; L4-adjacent, DP-16, ADR-0016)
-- Owner: PseudonymizationVault
--
...
-- downstream/async (BA-Q5), for rare authorized re-identification, and to make "delete = irreversible"
-- (ADR-0016) a single-row hard DELETE.

CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- ============================================================================
-- pseudonym_map -- identity<->pseudonym map. HARD DELETE on erasure (not soft-delete): the entire
-- point of "erasure = deleting the map row" (ADR-0016) is that no residual reversible value remains
-- once deleted -- a soft-deleted row with real_user_id_encrypted still present would not satisfy
-- "irreversible" under key-holder access. Encrypted at rest twice over: disk-level (NFR-S8 infra) AND
...
CREATE TABLE pseudonym_map (
    ...
    source_class          VARCHAR(40)  NOT NULL,                  -- OP_LOG | INCIDENT | EXT_TASK_LOG (audit-critical only)
...
-- ============================================================================
-- vault_access_audit -- every WRITE/READ/DELETE/REIDENTIFY_ATTEMPT (DP-16: least-privilege + audit).
...
COMMENT ON TABLE vault_access_audit IS
    'DP-16 least-privilege + audit. granted=false rows are the AUTH_PSEUDONYM_VAULT_ACCESS_DENIED (CRITICAL, security-page) evidence trail.';
