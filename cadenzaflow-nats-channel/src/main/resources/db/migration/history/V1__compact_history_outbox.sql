...
-- Owner: NatsHistoryEventHandler (writer) / HistoryOutboxRelay (reader+deleter)
...
--
-- IMPORTANT: this is the ONLY table increment 2 adds to the engine database. It does NOT touch
-- ACT_RU_*/ACT_HI_* (fork-owned, migrated by the engine's own Liquibase/DDL -- increment 1
-- precedent). This migration is applied by the TENANT alongside their
-- engine schema migrations (deployment concern: Flyway/Liquibase changelog integration).
...
CREATE TABLE compact_history_outbox (
    ...
    business_key        VARCHAR(255),                   -- CONFIDENTIAL/conditional PII (DP-2/DP-8); never in subject
    -- ARCH-Q1 (2026-07-18): payload transport = reference. payload_scalar carries the small/bounded
    ...
    -- NOT the full normalized ACT_HI multi-column shape. payload_large_ref is a soft pointer
    -- (no FK) to compact_history_outbox_payload for the rare large-byte
    -- case (typically EXT_TASK_LOG.errorDetails stack traces).
    payload_scalar      JSONB        NOT NULL,
    payload_large_ref   UUID,
    -- FINDING-001 (code review, 2026-07-20)/NEW-002 (2026-07-21): the engine's authoritative
...
-- ============================================================================
-- compact_history_outbox_payload -- optional large-byte companion (ARCH-Q1 "reference").
...
-- class is eventually cut over, BR-HDL-005). See LLD-Q2 for the
