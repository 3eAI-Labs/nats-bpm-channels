 ...
 * #1/#2/#3). Preserves the original message's headers
 ...
 *
 * <p><b>Core-NATS fallback dedup (resolved):</b>
 * {@code CoreNatsFallbackDedupTest} (a test source — NOT an {@code @link} because it is not on the
 * main javadoc classpath) empirically confirmed that the JetStream
 * {@code duplicate_window} also dedups messages published through the {@code connection.publish}
 * fallback path (not just the JetStream {@code publish} API): no additional safety note is
 * required for {@link DlqPublishOutcome#PUBLISHED_CORE_FALLBACK}.
 */
