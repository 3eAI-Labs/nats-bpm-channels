 ...
 * history-offload low-level design, verification #2 handed down from the HLD).
 * Builds the deterministic subject-mapped partitioning transform for the
 * {@code HISTORY} stream ({@code history.<engineId>.<class>.<processInstanceId>} publish-time subject →
 ...
 * and wildcard-token count so a future increment (or a different 3-token subject family) can reuse
 * it without a new class.
 *
 * <p><b>CODER-NOTE:</b> the low-level design describes the consumer filter as literally
...
public final class JetStreamSubjectPartitioner {
    ...
    /**
     * Resolves which replica owns which partition (either an explicit
