...
/**
 * Pure, no I/O — deterministic keyed-hash (BA-Q5). Called IN-TX by
 ...
 * algorithm/tenant-key version.
 *
 * <p><b>Irreversibility model:</b> the token itself is NOT
 ...
 * tenantKeyId, tenantKeyVersion)} and the class is required to be "pure, no I/O" — yet the config
 * design describes {@code tenantKeyId} as an OpenBao/deploy-secret
 * REFERENCE, not the secret bytes themselves. Resolving an OpenBao reference to
 ...
 * variable, per the secrets-management guideline) is responsible for resolving the
